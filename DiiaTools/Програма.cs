...
void ПобудуватиГрафПодій()
{
    ...
    var s1 = описСервісів.КонфігСервісів.Внутрішні.SelectMany(__ =>
    {
        ...
        //return черги.Select(_ => new Ребро<ВузелСистеми>(new(__.Key, ТипВузла.Сервіс), new(_, ТипВузла.Черга)));
    });
    var s2 = описСервісів.КонфігСервісів.Внутрішні.SelectMany(__ =>
    {
    ...
    var s3 = описСервісів.КонфігЧерг.Внутрішні.SelectMany(__ =>
    {
        var черги = __.Value.Теми ?? Array.Empty<string>();
        //return черги.Select(_ => new Ребро<ВузелСистеми>(new(__.Key, ТипВузла.Черга), new(_, ТипВузла.Тема)));
        return черги.Select(_ => new Ребро<ВузелСистеми>(new(_, ТипВузла.Тема), new(__.Key, ТипВузла.Черга)));
    });
    var s4 = описСервісів.КонфігТем.Внутрішні.SelectMany(__ =>
    {
        var черги = __.Value.Події ?? Array.Empty<string>();
        //return черги.Select(_ => new Ребро<ВузелСистеми>(new(__.Key, ТипВузла.Тема), new(_, ТипВузла.Подія)));
